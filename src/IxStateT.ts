 ...
 * @since 2.10.0
 */
export function iof<F extends URIS2>(F: Pointed2<F>): <S, E, A>(a: A) => IxStateT2<F, S, S, E, A>
export function iof<F extends URIS>(F: Pointed1<F>): <S, A>(a: A) => IxStateT1<F, S, S, A>
export function iof<F>(F: Pointed<F>): <S, A>(a: A) => IxStateT<F, S, S, A>
export function iof<F>(F: Pointed<F>): <I, A>(a: A) => IxStateT<F, I, I, A> {
  return (a) => (i) => F.of([a, i])
}
/**
 * @category Pointed
